...
use App\Models\ApiAttendanceTemp;
use Carbon\Carbon;
use DB;
...
Route::post('/sync_location_fmc', function(Request $request){
    ...
    if(!$validate_bioloc){
        ...
        $update_query = BioLocation::where('location', $request->location)->where('serial_number', trim($request->serial_number))->update(['status' => 1, 'updated_at' => Carbon::now()->format('Y-m-d H:m:s')]);
        $new_id_location = BioLocation::where('location', $request->location)->where('status', 1)->orderByDesc('id')->first()->id;
...
Route::post('/sync_attendance_fmc', function(Request $request){
    ...
    try {
        ApiAttendanceTemp::create($payload);
